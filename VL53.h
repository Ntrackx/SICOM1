...
#define      SHADOW_PHASECAL_RESULT__REFERENCE_PHASE_LO                                  0x0FFF
   ...
   // RangeInvalid              =  14: can't actually be returned by API (range can never become negative, even after correction)
    ...
    uint8_t getAddress(uint8_t VL_ADDR);

    bool vl_init(void);

    bool setDistanceMode(uint8_t mode, uint8_t VL_ADDR);
    uint8_t getDistanceMode(uint8_t VL_ADDR);

    bool setMeasurementTimingBudget(uint32_t budget_us, uint8_t VL_ADDR);
    uint32_t getMeasurementTimingBudget(uint8_t VL_ADDR);

    void startContinuous(uint32_t period_ms, uint8_t VL_ADDR);
    void stopContinuous(uint8_t VL_ADDR);
    uint16_t read(uint8_t ADDR);
    uint16_t readRangeContinuousMillimeters(bool blocking); // alias of read()
